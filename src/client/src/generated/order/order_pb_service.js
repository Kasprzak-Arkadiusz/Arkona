...
Order.GetUserOrders = {
  ...
  requestStream: false,
  responseStream: false,
  requestType: order_pb.GetUserOrdersRequest,
  responseType: order_pb.GetUserOrdersResponse
};
...
OrderClient.prototype.getUserOrders = function getUserOrders(requestMessage, metadata, callback) {
  if (arguments.length === 2) {
    callback = arguments[1];
  }
  var client = grpc.unary(Order.GetUserOrders, {
    request: requestMessage,
    host: this.serviceHost,
    metadata: metadata,
    transport: this.options.transport,
